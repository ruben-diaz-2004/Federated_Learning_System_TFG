...
Puede usarse de dos formas:
  ...
  1. Como módulo importable:
       ...
       result = run_syft_pipeline(data_dir="...", model_out="...", cfg={...})
...
# ══════════════════════════════════════════════════════════════════════════════
# Función exportable — toda la lógica vive aquí
# ══════════════════════════════════════════════════════════════════════════════
...
def run_syft_pipeline(
    ...
    image_asset     = sy.Asset(name="classification_images",
                               data=raw_data_path, mock=mock_data)
    medical_dataset = sy.Dataset(
        name="Medical Images Dataset",
        description="Total images: 1500. Labels available: ['class_A', 'class_B']. "
    ...
    def remote_preprocessing_and_training(raw_data_dict):
        ...
        try:
            ...
            train_loader = DataLoader(
                train_split, batch_size=batch_size, sampler=sampler,
            ...
            test_loader = DataLoader(
                test_split, batch_size=batch_size, shuffle=False,
                collate_fn=collate_fn, num_workers=0,
            )
    ...
    result_pointer = scientist_client.code.remote_preprocessing_and_training(
        raw_data_dict=asset
    )
